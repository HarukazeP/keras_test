...
#2つのベクトルの平均二乗誤差
def calc_mean_squared_error(pred_vec, ans_vec):
    diff=ans_vec-pred_vec
...
def print_and_get_rank(pred_vec, choices, fname, vec_dict, ft_path, bin_path, id_to_word):
    ...
    for i in range(len_words):
        if i!=0:
            word=id_to_word[i]
            dict_all[word]=calc_mean_squared_error(pred_vec, get_ft_vec(word, vec_dict, ft_path, bin_path))
    for x in choi_list:
        dict_ch[x]=calc_mean_squared_error(pred_vec, get_ft_vec(x, vec_dict, ft_path, bin_path))
...
def calc_acc(rank_file, ans_list, preds_list, top_list, choice_list, vec_dict, ft_path, bin_path):
    ...
    with open(rank_file,'r') as rank:
        for line in rank:
            ...
            ans=ans_list[sent_i]
            ans_sim=calc_mean_squared_error(preds_list[sent_i], get_ft_vec(ans, vec_dict, ft_path, bin_path))
...
save_path=save_path+'Mean_error_TEST_'
