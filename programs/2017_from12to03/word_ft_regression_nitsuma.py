...
def build_model(len_words, embedding_matrix):
    ...
    f_1=Bidirectional(LSTM(128, return_sequences=True))(f_emb)    #最後の出力じゃないときはreturn_sequences=Trueが必須
    f_layer=Bidirectional(LSTM(128))(f_1)
    ...
    r_1=Bidirectional(LSTM(128, return_sequences=True))(r_emb)
    r_layer=Bidirectional(LSTM(128))(r_1)
    
    merged_layer=concatenate([f_layer, r_layer])
    
    den1=Dense(vec_size,activation='relu')(merged_layer)
    out_layer=Dense(vec_size,activation='relu')(den1)
