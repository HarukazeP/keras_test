...
#読み込むもの
train_path = './corpus/miniWiki_tmp8.txt'     #学習データ
test_path = './corpus/center_goudouzemi.txt'     #答えつきテストデータ
ch_path= './corpus/center_goudouzemi_choices.txt'     #選択肢つきテストデータ
...
model.fit([f_X,r_X], y, batch_size=128, epochs=10)
print_time('fit end')
...
#テストデータへ読み込みと前処理，ベクトル化
#テストデータは1行1問で1行に<>が1つのみ
test_data = open(test_path).read().lower()
#ans_data= open(ans_path).read().lower()
ch_data= open(ch_path).read().lower()
all_lines = test_data.split("\n")
#ans_lines = ans_data.split("\n")
...
for i in range(sents_num):
    test_f_x = np.zeros((1, maxlen_words, len_words))
    test_r_x = np.zeros((1, maxlen_words, len_words))
    for t, word in enumerate(test_f_sentences[i]):
        if(len(test_f_sentences[i])<maxlen_words):
            test_f_x[0, t+maxlen_words-len(test_f_sentences[i]), search_word_indices(word)] = 1.
        else:
            test_f_x[0, t, search_word_indices(word)] = 1.
    for t, word in enumerate(test_r_sentences[i]):
        if(len(test_f_sentences[i])<maxlen_words):
            test_r_x[0, t+maxlen_words-len(test_r_sentences[i]), search_word_indices(word)] = 1.
        else:
            test_r_x[0, t, search_word_indices(word)] = 1.
...
def calc_samp1word(pred, ans, list_rank):
    ...
    with open(today_str+'_sampOK.txt', 'a') as sOK:
        with open(today_str+'_sampNG.txt', 'a') as sNG:
            out='preds= '+preds+' : '+rank_pred+'     '+'ans= '+ans+' : '+rank_ans+'\n'
            if preds==ans:
                sOK.write(out)
                OK_num=1
            else:
